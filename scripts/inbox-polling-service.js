...
async function pollGathernAccount(account) {
    ...
    try {
        ...
        for (const chat of chats) {
            ...
            try {
                const [placeholders] = await db.execute(`SELECT id FROM platform_messages 
           WHERE thread_id = ? AND platform = ? AND message_text = ? AND platform_msg_id LIKE 'sent-%' AND is_from_me = 1`, [threadId, 'gathern', text]);
...
async function pollAirbnbAccount(account) {
    ...
    try {
        ...
        for (const thread of threads) {
            ...
            let text = '';
            if (latestMsg.text) {
                if (typeof latestMsg.text === 'string')
                    text = latestMsg.text;
                else if (latestMsg.text.accessibilityText)
                    text = latestMsg.text.accessibilityText;
                else if (Array.isArray(latestMsg.text.components))
                    text = latestMsg.text.components.map((c) => c.text).join(' ');
            }
            if (!text)
                text = latestMsg.body || latestMsg.message || '';
            const guestName = thread.users?.find((u) => u.type === 'GUEST')?.name || 'Guest';
            const msgId = latestMsg.id || thread.latestMessageId || 'unknown';
            let finalIsFromMe = isFromMe ? 1 : 0;
            try {
                const [placeholders] = await db.execute(`SELECT id FROM platform_messages 
           WHERE thread_id = ? AND platform = ? AND message_text = ? AND platform_msg_id LIKE 'sent-%' AND is_from_me = 1`, [threadId, 'airbnb', text]);
                if (placeholders && placeholders.length > 0) {
                    finalIsFromMe = 1;
                    await db.execute(`DELETE FROM platform_messages WHERE id = ?`, [placeholders[0].id]);
                }
...
async function runPoll() {
    const db = getPool();
    try {
        const [accounts] = await db.query('SELECT * FROM browser_accounts WHERE is_active = 1');
