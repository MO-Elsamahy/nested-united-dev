...
async function getBrowserAccounts(): Promise<BrowserAccount[]> {
  try {
    ...
    console.error("[BrowserAccounts] Error:", error);
    return [];
  }
}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-red-50 p-3 rounded-xl flex items-center justify-center">
              <AirbnbIcon size={38} className="text-red-500" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="bg-blue-900 p-3 rounded-xl flex items-center justify-center">
              <GathernIcon size={24} className="text-white" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="divide-y divide-gray-100">
            {accounts.map((account) => (
              <div
                key={account.id}
                className={`p-5 hover:bg-gray-50/50 transition-colors ${account.has_unread_notifications ? 'bg-red-50/30' : ''
                  }`}
              >
                <div className="flex items-center justify-between">
                  {/* Account Info */}
                  <div className="flex items-center gap-4">
                    ...
                    <div className={`relative w-14 h-14 rounded-2xl flex items-center justify-center ${account.platform === 'airbnb'
                      ...
                      {account.platform === 'whatsapp' ? (
                        ...
                        <AirbnbIcon size={42} className="text-white" />
                      ) : account.platform === 'gathern' ? (
                  ...
                  {/* Actions */}
                  <div className="flex items-center gap-3">
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm ${account.is_active
